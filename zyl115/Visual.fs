...
module Visual =
    ...
    let RunVisualWithFlagsOut paras src =
        let main, post = VData.GETWRAPPER paras.InitRegs paras.InitFlags paras.MemReadBase paras.InitMM
    ...
    let RunVisualWithMemOut paras src =
        let main, post = VData.GETWRAPPER paras.InitRegs paras.InitFlags paras.MemReadBase paras.InitMM
        let res = RunVisual {paras with Prelude=main; Postlude=post} src
        match res with
        | Error e -> failwithf "Error reading Visual Log %A" e
        | Ok ({ Regs=_; State={VMemData = mem}} as vso) -> mem,vso
