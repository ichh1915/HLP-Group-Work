...
type Instr =
    ...
    | IMOV of MV2.Instr
    | ISFT of SF2.Instr
    | IFILL of EQUFILL.Instr
    | IEQU of EQUFILL.Instr      
...
let storeDataReturnMM' addr source byte dataPath= 
    storeDataReturnMM addr dataPath source byte
...
let execFILL (stradr:uint32) (ins:Instr) (dp:DataPath<Instr>): Result<DataPath<Instr>, ErrInstr> = 
    let numofWords =
        match ins with
        |IFILL ins' when CheckCond dp ins'.Cond-> ins'.Express/4u
        |_->0u
    
    let rec loop stradr' numofWords' dp' = 
        match numofWords' with
        |numofWords' when numofWords'>0u ->   dp'|>storeDataReturnMM' stradr 0u false 
                                                 |>Result.map (fun k -> PCPlus4 {dp with MM = k})
                                                 |>function |Ok (d)->d |Error k->failwithf "Error"
                                                 |>loop (stradr'+ 4u) (numofWords'- 4u)
        |_->dp'
        
    Ok (loop stradr numofWords dp) 


let execEQU (ins:Instr) (dp:DataPath<Instr>) (ld:LineData): Result<LineData, ErrInstr> =
    let exp = 
        match ins with
        |IEQU ins' when CheckCond dp ins'.Cond->ins'.Express
        |_->0u 
    
    match (ld,exp) with
    |({Label=Some lb;SymTab=Some sym},e) when e<>0u->Ok {LoadAddr=ld.LoadAddr;Label=ld.Label;SymTab=Some (Map.add lb exp sym);OpCode=ld.OpCode;Operands=ld.Operands}
    |({Label=Some lb;SymTab=Some sym},e) when e=0u->Ok ld
    |(_,_)->Error (ERRTOPLEVEL "EQU error")
