...
module Visual =
    ...
    let RunVisualBaseWithLocksCached (paras: Params) src :Result<VisOutput, string list>=
    ...
    /// convert NZCV status flags to 4 bits PC content
    let FlagsToPC (fl:Flags): uint32 = 
        let tobit = function |true ->1u |false ->0u 
        match fl with { FN=a;FZ=b;FC=c;FV=d} -> (((a|>tobit)<<<3) |||((b|>tobit)<<<2)) ||| (((c|>tobit)<<<1) ||| (d|>tobit))
